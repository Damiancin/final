...
import { buildRoutineForWeek, BLOCK_COLORS } from './data/defaultRoutine';
...
function App() {
  ...
  const toggleGym = (day: string) => setGymDays((prev) => prev.map((g) => g.day === day ? { ...g, completed: !g.completed } : g));
  ...
  const handleAddBlock = async () => {
    ...
    if (end <= start) return;
    const block: AgendaBlock = { id: `blk-${Date.now()}`, date: addDate || selectedDate, person: addPerson, label: addLabel.trim(), type: addBlockType, start, end, notes: addNotes.trim(), color: BLOCK_COLORS[addBlockType] || BLOCK_COLORS.Otro, is_recurring: false };
  ...
  const handleSaveEdit = async () => {
    ...
    if (end <= start) return;
    const updated: AgendaBlock = { ...editingBlock, label: editLabel.trim(), date: editDate, person: editPerson, type: editBlockType, start, end, notes: editNotes.trim(), color: BLOCK_COLORS[editBlockType] || BLOCK_COLORS.Otro };
  ...
  const renderBlock = useCallback((block: AgendaBlock) => {
    const top = (block.start - 8) * 48, height = (block.end - block.start) * 48;
    return (
      <div key={block.id} className={`absolute left-1 right-1 rounded-lg border ${block.color} cursor-pointer transition-all hover:brightness-125 group`} style={{ top: `${top + 24}px`, height: `${Math.max(height - 4, 16)}px` }} onClick={() => openEditModal(block)}>
        <div className="p-1.5 overflow-hidden h-full relative">
          <p className="text-xs font-medium text-zinc-100 truncate">{block.label}</p>
          <p className="text-[10px] text-zinc-400 mt-0.5">{formatHour(block.start)} - {formatHour(block.end)}</p>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-zinc-900/50 border border-zinc-800 rounded-2xl">
              <div className="px-5 py-4 border-b border-zinc-800 flex items-center justify-between">
                <div className="flex items-center gap-2"><Dumbbell className="w-4 h-4 text-emerald-400" /><h3 className="text-sm font-semibold text-zinc-200">Gimnasio semanal</h3></div>
                <span className="text-[10px] text-zinc-500 bg-zinc-800 px-2 py-0.5 rounded-full">{gymCompleted}/7</span>
              </div>
              <div className="p-4">
                <div className="grid grid-cols-7 gap-1.5 mb-4">
                  {gymDays.map((g) => (
                    <button key={g.day} onClick={() => toggleGym(g.day)} className={`flex flex-col items-center py-2 rounded-lg border transition-all ${g.completed ? 'bg-emerald-500/15 border-emerald-500/30 text-emerald-400' : 'bg-zinc-800/50 border-zinc-800 text-zinc-500 hover:border-zinc-700'}`}>
                      <span className="text-[10px] font-medium">{g.day}</span><span className="text-[8px] mt-0.5 opacity-70">{g.type}</span>
                    </button>
                  ))}
                </div>
                <div className="space-y-1.5 mb-4">
                  {gymDays.map((g) => (
                    <div key={g.day} className="flex items-center gap-2 text-[10px]">
                      ...
                      <span className="text-zinc-500 w-6">{g.day}</span><span className="text-zinc-400">{g.detail}</span>
                    </div>
                  ))}
                </div>
                <div className="h-1.5 bg-zinc-800 rounded-full overflow-hidden"><div className="h-full bg-gradient-to-r from-emerald-500 to-teal-400 rounded-full transition-all duration-500" style={{ width: `${(gymCompleted / 7) * 100}%` }} /></div>
                <p className="text-[10px] text-zinc-500 mt-2 text-center">{gymCompleted === 7 ? 'Semana completada' : `${7 - gymCompleted} días restantes`}</p>
              </div>
            </div>
