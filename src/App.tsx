...
// Supabase CRUD
async function sbLoadBlocks() {
  if (!supabase) return null;
  const { data, error } = await supabase.from('agenda_blocks').select('*');
  return error ? null : data as AgendaBlock[];
}

async function sbInsertBlock(b: AgendaBlock) {
  if (!supabase) return;
  await supabase.from('agenda_blocks').insert([b]);
}

async function sbUpdateBlock(b: AgendaBlock) {
  if (!supabase) return;
  await supabase.from('agenda_blocks').update(b).eq('id', b.id);
}

async function sbDeleteBlock(id: string) {
  if (!supabase) return;
...
async function sbLoadEntries() {
  ...
  return error ? null : data as QuickEntry[];
}

async function sbInsertEntry(e: QuickEntry) {
  if (!supabase) return;
  await supabase.from('quick_entries').insert([e]);
}

async function sbUpdateEntry(e: QuickEntry) {
  if (!supabase) return;
  await supabase.from('quick_entries').update(e).eq('id', e.id);
}

async function sbDeleteEntry(id: string) {
  if (!supabase) return;
  await supabase.from('quick_entries').delete().eq('id', id);
}
...
function App() {
  ...
  const [customBlocks, setCustomBlocks] = useState<AgendaBlock[]>(() =>
    lsGet('agenda-custom-blocks', [])
  );

  const [deletedRecurring, setDeletedRecurring] = useState<string[]>(() =>
    lsGet('agenda-deleted-recurring', [])
  );

  const [gymDays, setGymDays] = useState<GymDay[]>(() =>
    lsGet('agenda-gym-state', INITIAL_GYM)
  );

  const [entries, setEntries] = useState<QuickEntry[]>(() =>
    lsGet('agenda-quick-entries', [])
  );
  ...
  useEffect(() => {
    sbLoadBlocks().then((data) => {
      if (data) {
        const c = data.filter((b) => !b.is_recurring);
        setCustomBlocks(c);
        lsSet('agenda-custom-blocks', c);
      }
    });

    sbLoadEntries().then((data) => {
      if (data) {
        setEntries(data);
        lsSet('agenda-quick-entries', data);
      }
    });
  }, []);

  useEffect(() => {
    lsSet('agenda-custom-blocks', customBlocks);
  }, [customBlocks]);

  useEffect(() => {
    lsSet('agenda-quick-entries', entries);
  }, [entries]);

  useEffect(() => {
    lsSet('agenda-gym-state', gymDays);
  }, [gymDays]);

  useEffect(() => {
    lsSet('agenda-deleted-recurring', deletedRecurring);
  }, [deletedRecurring]);

  const routineBlocks = buildRoutineForWeek(weekStart).filter(
    (b) => !deletedRecurring.includes(b.id)
  );
  ...
  const toggleGym = (day: string) =>
    setGymDays((prev) =>
      prev.map((g) =>
        g.day === day ? { ...g, completed: !g.completed } : g
      )
    );
  ...
  const toggleEntryDone = async (id: string) => {
    ...
    const e = updated.find((entry) => entry.id === id);
    if (e) await sbUpdateEntry(e);
  };
  ...
  const handleAddBlock = async () => {
    ...
    const block: AgendaBlock = {
      ...
      notes: addNotes.trim(),
      color: getSavedPersonColor(addPerson),
      is_recurring: false,
    };
  ...
  const handleSaveEdit = async () => {
    ...
    const updated: AgendaBlock = {
      ...
      color: getSavedPersonColor(editPerson),
    };
    ...
    if (editingBlock.is_recurring) {
      ...
      setCustomBlocks((prev) => [...prev, nb]);
      await sbInsertBlock(nb);
    } else {
      setCustomBlocks((prev) =>
        prev.map((b) => (b.id === editingBlock.id ? updated : b))
      );

      await sbUpdateBlock(updated);
    }
  ...
  const handleDeleteBlock = async () => {
    ...
    if (editingBlock.is_recurring) {
      ...
      await sbDeleteBlock(editingBlock.id);
    }
  ...
  const getEntryIcon = (t: QuickEntry['type']) =>
    ENTRY_TYPES.find((x) => x.value === t)?.icon ?? MessageSquare;

  const getEntryColor = (t: QuickEntry['type']) =>
    ENTRY_TYPES.find((x) => x.value === t)?.color ?? 'text-zinc-400';

  const getPersonLabel = (p: 'tú' | 'joss' | 'ambos') =>
    p === 'tú' ? 'Damián' : p === 'joss' ? 'Joss' : 'Ambos';
  ...
  const renderBlock = useCallback((block: AgendaBlock, column: 'damian' | 'joss') => {
    ...
    return (
      <div
        ...
        style={{
          top: `${top + 24}px`,
          height: `${Math.max(height - 4, 16)}px`,
        }}
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100">
              ...
              {modalTab === 'note' ? (
                <>
                  <div>
                    ...
                    <select
                      value={newType}
                      onChange={(e) => setNewType(e.target.value as QuickEntry['type'])}
                      className="mt-1 w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-xs text-zinc-200 outline-none focus:border-emerald-500"
                    >
                      {ENTRY_TYPES.map((t) => (
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Ej: Hoy salí tarde del trabajo"
                      className="mt-1 w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-xs text-zinc-200 outline-none focus:border-emerald-500 placeholder:text-zinc-600"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <select
                      value={newPerson}
                      onChange={(e) => setNewPerson(e.target.value as QuickEntry['person'])}
                      className="mt-1 w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-xs text-zinc-200 outline-none focus:border-emerald-500"
                    >
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 py-2.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 disabled:bg-zinc-700 disabled:text-zinc-500 text-white text-xs font-medium transition-colors"
                  >
                ...
                <>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Ej: Partido de basket"
                      className="mt-1 w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-xs text-zinc-200 outline-none focus:border-emerald-500 placeholder:text-zinc-600"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <select
                      value={addPerson}
                      onChange={(e) => setAddPerson(e.target.value as AgendaBlock['person'])}
                      className="mt-1 w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-xs text-zinc-200 outline-none focus:border-emerald-500"
                    >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Ej: Llevar documentos"
                      className="mt-1 w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-xs text-zinc-200 outline-none focus:border-emerald-500 placeholder:text-zinc-600"
                    />
                  </div>
          ...
          <div className="bg-zinc-900 border border-zinc-700 rounded-2xl w-full max-w-md shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-5 space-y-4">
              ...
              <div>
                ...
                <select
                  value={editPerson}
                  onChange={(e) => setEditPerson(e.target.value as AgendaBlock['person'])}
                  className="mt-1 w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-xs text-zinc-200 outline-none focus:border-emerald-500"
                >
                  <option value="damian">Damián</option>
                  <option value="joss">Joss</option>
              ...
              {editingBlock.is_recurring && (
                <p className="text-[10px] text-amber-400/70">
                  Bloque recurrente. Al guardar se creará una copia editable solo para esta fecha.
                </p>
              )}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        <div className="flex items-center gap-2 mb-6 overflow-x-auto pb-2">
          <button
            onClick={goPrevWeek}
            className="w-8 h-8 rounded-lg bg-zinc-800 border border-zinc-700 flex items-center justify-center hover:bg-zinc-700 transition-colors shrink-0"
          >
          ...
          <button
            onClick={goTodayWeek}
            className="px-3 h-8 rounded-lg bg-zinc-800 border border-zinc-700 text-[10px] text-zinc-400 hover:bg-zinc-700 hover:text-zinc-200 transition-colors shrink-0 font-medium"
          >
            Hoy
          </button>
              ...
              <div className="px-5 py-4 border-b border-zinc-800 flex items-center justify-between">
                <div>
                  <h2 className="text-sm font-semibold text-zinc-200">
                    {formatDayFull(selectedDateObj)} — {selectedDate}
                  </h2>
              ...
              <div className="flex overflow-x-auto">
                <div className="w-14 shrink-0 border-r border-zinc-800">
                  {HOURS.map((hour) => (
                    <div
                      key={hour}
                      className="h-12 flex items-start justify-end pr-2 pt-0.5 text-[10px] text-zinc-600"
                    >
                    ...
                    >
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <input
                          value={g.type}
                          onChange={(e) =>
                            updateGymDay(g.day, 'type', e.target.value)
                          }
                      ...
                      <input
                        value={g.detail}
                        onChange={(e) =>
                          updateGymDay(g.day, 'detail', e.target.value)
                        }
              ...
              <div className="p-4 space-y-2 max-h-80 overflow-y-auto">
                {entries.length === 0 && (
                  <p className="text-xs text-zinc-600 text-center py-4">
                    Sin entradas. Agrega una nota o pendiente.
                  </p>
                )}
                ...
                {entries.map((entry) => {
                  ...
                  return (
                    <div
                      key={entry.id}
                      className="flex items-start gap-2.5 group hover:bg-zinc-800/50 rounded-lg p-2 -m-1 transition-colors"
                    >
                      <button
                        onClick={() =>
                          entry.type === 'pendiente' && toggleEntryDone(entry.id)
                        }
                      ...
                      <div className="flex-1 min-w-0">
                        <p
                          className={`text-xs ${
                            entry.type === 'pendiente' && entry.done
